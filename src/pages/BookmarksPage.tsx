...
export default function BookmarksPage() {
  ...
  return (
    <PageTransition className="max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold text-foreground mb-6">ブックマーク</h1>
      {bookmarked.length === 0 ? (
        <EmptyState icon={Bookmark} title="ブックマークがありません" description="記事をブックマークすると、ここに表示されます" action={<Link to="/" className="inline-flex items-center gap-2 px-4 py-2 bg-accent-600 hover:bg-accent-500 text-on-accent text-sm font-medium rounded-lg transition-colors">記事を探す</Link>} />
      ) : (
